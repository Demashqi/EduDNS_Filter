...
import com.tus.DNSResolver.users_manager.repository.ApiGroupRepository;
...
public class DataInitializer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        if (apiGroupRepository.count() == 0) {
            ...
            ApiEndpoint addBlockedDomain = new ApiEndpoint("POST", "/api/admin/blocklist", "addBlockedDomain", blocklistGroup, Set.of(Role.TEACHER, Role.ADMIN));
            ApiEndpoint getBlockedDomains = new ApiEndpoint("GET", "/api/admin/blocklist", "getBlockedDomains", blocklistGroup, Set.of(Role.TEACHER,Role.ADMIN));
            ApiEndpoint getBlockedDomainById = new ApiEndpoint("GET", "/api/admin/blocklist/{id}", "getBlockedDomainById", blocklistGroup, Set.of(Role.TEACHER,Role.ADMIN));
            ApiEndpoint deleteBlockedDomainByName = new ApiEndpoint("DELETE", "/api/admin/blocklist/name/{domain}", "deleteBlockedDomainByName", blocklistGroup, Set.of(Role.TEACHER,Role.ADMIN));
